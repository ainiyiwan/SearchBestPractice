        ...
        for (String key : keys) {
            ...
            try {
                if (value == null) {
                    ...
                    TLog.d(TAG, String.format("Con't support save this type:%s, value:%s, key:%s",
                            type, value, key));
                }
            } catch (IllegalArgumentException e) {
                TLog.d(TAG, "Save error:" + e.getMessage());
            }
        }
    ...
    private static <T> void buildValuesToMap(Class<?> clx, T t, String preFix,
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                TLog.d(TAG, "buildValuesToMap error:" + e.getMessage());
                continue;
            }
        ...
        for (Field field : fields) {
            ...
            if (value != null) {
                try {
                    ...
                    e.printStackTrace();
                    TLog.d(TAG, String.format("Set field error, Key:%s, type:%s, value:%s",
                            key, fieldType, value));
                }
            } else {
                TLog.d(TAG, String.format("Get field value error, Key:%s, type:%s",
                        key, fieldType));
            }
        }
